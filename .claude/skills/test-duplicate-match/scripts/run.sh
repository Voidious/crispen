#!/usr/bin/env bash
# Automates crispen's manual DuplicateExtractor / match-function test loop.
# See ../SKILL.md for what this does, how to set up a test case first, and
# the cost/retry rules below. Adapted from ../../test-patch-rewrite/scripts/run.sh
# (same diff mechanism and retry philosophy) — no FileLimiter-generated-dir
# cleanup here since DuplicateExtractor never creates new files/packages.
#
# Usage:
#   run.sh --target <file> --branch-suffix <suffix> \
#          [--max-retries N] [--success-labels "label1,label2,..."]
#
#   --target          repo-relative path, e.g. crispen/_livetest_target.py
...
#                      whether the LLM path was actually exercised.
#                      Default: "match existing:,duplicate extracted:"

set -euo pipefail

MAX_RETRIES=3
SUCCESS_LABELS="match existing:,duplicate extracted:"
TARGET_FILE=""
BRANCH_SUFFIX=""
...
if [ -n "$(git status --porcelain)" ]; then
  echo "error: working tree is not clean. Commit your test-case setup (see SKILL.md 'Setting up a test case') before running this script." >&2
  exit 1
fi
...
# SUCCESS_LABELS, e.g. "match existing:,duplicate extracted:" -> sum of
# those two lines' trailing numbers. Missing lines count as 0.
sum_success_labels() {
...
while [ "$attempt" -le "$MAX_RETRIES" ]; do
  ...
  # just missing coverage on scratch test-case files (which aren't meant to
  # be kept — see SKILL.md).
  ...
  if [ "$success_count" -eq 0 ]; then
    echo "!! zero matching LLM outcomes this attempt — retrying (veto/match is nondeterministic)"
    attempt=$((attempt + 1))
    continue
  fi
