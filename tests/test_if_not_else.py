...
def _prepare_transformer(source: str, ranges, cst, MetadataWrapper, IfNotElse):
    if ranges is None:
        ranges = [(1, 100)]
...
def _apply(source: str, ranges=None) -> str:
    wrapper, transformer = _prepare_transformer(
        source, ranges, cst, MetadataWrapper, IfNotElse
    )
