...
def _parse_and_assert_single_entity(source: str, expected_kind: EntityKind) -> object:
    entities = parse_entities(source)
    assert len(entities) == 1, f"expected 1 entity, got {len(entities)}"
    e = entities[0]
    assert e.kind == expected_kind, f"expected kind {expected_kind}, got {e.kind}"
    return e


def test_parse_single_function():
    source = "def foo():\n    pass\n"
    e = _parse_and_assert_single_entity(source, EntityKind.FUNCTION)
    assert e.name == "foo"
...
def _assert_single_top_level_entity(entities, expected_count=1):
    assert len(entities) == expected_count
    entity = entities[0]
    assert entity.kind == EntityKind.TOP_LEVEL
    return entity
...
def _parse_single_entity(source: str, expected_start_line: int):
    entities = parse_entities(source)
    assert len(entities) == 1
    e = entities[0]
    assert e.start_line == expected_start_line
    return e
...
def test_parse_decorated_function_uses_decorator_line():
    ...
    e = _parse_single_entity(source, 1)
    assert e.name == "foo"
