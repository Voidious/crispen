...
def _assert_single_entity(source: str, expected_kind: EntityKind) -> object:
...
def test_parse_single_function():
    source = "def foo():\n    pass\n"
    e = _assert_single_entity(source, EntityKind.FUNCTION)
    assert e.name == "foo"
...
def _assert_single_entity_start_line(source: str, expected_line: int):
    entities = parse_entities(source)
    assert len(entities) == 1
    e = entities[0]
    assert e.start_line == expected_line
    return e


def test_parse_decorated_function_uses_decorator_line():
    source = "# Comment\n@decorator\ndef foo():\n    pass\n"
    e = _assert_single_entity_start_line(source, 1)
