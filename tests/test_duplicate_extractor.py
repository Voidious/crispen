...
def _setup_mock_client_for_invalid_extract(mock_anthropic, verbose):
    mock_client = MagicMock()
    mock_anthropic.Anthropic.return_value = mock_client
    mock_anthropic.APIError = Exception
    mock_client.messages.create.side_effect = [
...
def _make_invalid_assembled_extractor(monkeypatch, verbose=True):
    ...
    with (
        ...
        return _setup_mock_client_for_invalid_extract(mock_anthropic, verbose)
...
def _setup_mock_client_with_verify_retry(mock_anthropic, helper):
    mock_client = MagicMock()
    mock_anthropic.Anthropic.return_value = mock_client
    mock_anthropic.APIError = Exception
    mock_client.messages.create.side_effect = [
...
def test_llm_verify_rejects_then_retries_verbose(monkeypatch, capsys):
    ...
    helper = "def _helper(data):\n    pass\n"
    with patch("crispen.llm_client.anthropic") as mock_anthropic:
        _setup_mock_client_with_verify_retry(mock_anthropic, helper)
        de = DuplicateExtractor(
