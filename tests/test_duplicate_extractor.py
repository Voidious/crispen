...
def _setup_two_group_mocks(mock_anthropic):
    mock_client = MagicMock()
    mock_anthropic.Anthropic.return_value = mock_client
    mock_anthropic.APIError = Exception
    mock_client.messages.create.side_effect = [
...
def test_undefined_helper_in_combined_two_groups_one_dropped(monkeypatch):
    ...
    with (
        ...
        mock_client = _setup_two_group_mocks(mock_anthropic)
        de = DuplicateExtractor(
...
def _setup_retry_mocks(mock_anthropic, helper, veto_reason="same logic"):
    mock_client = MagicMock()
    mock_anthropic.Anthropic.return_value = mock_client
    mock_anthropic.APIError = Exception
    mock_client.messages.create.side_effect = [
