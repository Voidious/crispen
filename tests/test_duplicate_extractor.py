...
def _make_successful_extractor(monkeypatch, side_effects):
...
def test_successful_extraction_has_two_blank_lines(monkeypatch):
    ...
    de = _make_successful_extractor(monkeypatch, side_effects)
...
def _make_cross_class_staticmethod_rejected_responses(
    helper, _make_veto_response, _make_extract_response, _make_verify_response
):
    return [
        _make_veto_response(True),
        _make_extract_response(
...
def test_cross_class_staticmethod_placement_rejected(monkeypatch):
    ...
    with patch("crispen.llm_client.anthropic") as mock_anthropic:
        ...
        mock_anthropic.Anthropic.return_value = mock_client
        mock_anthropic.APIError = Exception
        responses = _make_cross_class_staticmethod_rejected_responses(
            helper, _make_veto_response, _make_extract_response, _make_verify_response
        )
        mock_client.messages.create.side_effect = responses
        de = DuplicateExtractor([(3, 5)], source=source)
