...
def _setup_cross_class_staticmethod_rejection_mocks(mock_anthropic, helper):
    mock_client = MagicMock()
    mock_anthropic.Anthropic.return_value = mock_client
    mock_anthropic.APIError = Exception
    responses = [
...
def test_cross_class_staticmethod_placement_rejected(monkeypatch):
    ...
    helper = "def _helper(data):\n    pass\n"
    with patch("crispen.llm_client.anthropic") as mock_anthropic:
        mock_client = _setup_cross_class_staticmethod_rejection_mocks(
...
def _setup_retry_mocks(mock_anthropic, helper):
    mock_client = MagicMock()
    mock_anthropic.Anthropic.return_value = mock_client
    mock_anthropic.APIError = Exception
    mock_client.messages.create.side_effect = [
...
def test_extraction_retry_on_alg_failure_verbose(monkeypatch, capsys):
    ...
    helper = "def _helper(data):\n    pass\n"
    with patch("crispen.llm_client.anthropic") as mock_anthropic:
        mock_client = _setup_retry_mocks(mock_anthropic, helper)
        de = DuplicateExtractor(
