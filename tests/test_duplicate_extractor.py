...
def _make_successful_extraction_setup(monkeypatch):
...
def test_successful_extraction_has_two_blank_lines(monkeypatch):
    de = _make_successful_extraction_setup(monkeypatch)
...
def _setup_mock_anthropic_retry_side_effects(mock_anthropic, helper):
    mock_client = MagicMock()
    mock_anthropic.Anthropic.return_value = mock_client
    mock_anthropic.APIError = Exception
    mock_client.messages.create.side_effect = [
...
def test_extraction_retry_on_alg_failure_verbose(monkeypatch, capsys):
    ...
    helper = "def _helper(data):\n    pass\n"
    with patch("crispen.llm_client.anthropic") as mock_anthropic:
        _setup_mock_anthropic_retry_side_effects(mock_anthropic, helper)
        de = DuplicateExtractor(
