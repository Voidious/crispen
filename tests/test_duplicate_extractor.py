...
def _make_duplicate_extractor_with_helper(monkeypatch, source, helper):
...
def test_successful_extraction_has_two_blank_lines(monkeypatch):
    ...
    de = _make_duplicate_extractor_with_helper(monkeypatch, source, helper)
...
def _configure_mock_client_with_single_replacement(mock_anthropic):
    mock_client = MagicMock()
    mock_anthropic.Anthropic.return_value = mock_client
    mock_anthropic.APIError = Exception
    mock_client.messages.create.side_effect = [
...
def test_wrong_replacement_count_skipped(monkeypatch):
    monkeypatch.setenv("ANTHROPIC_API_KEY", "test-key")
    with patch("crispen.refactors.duplicate_extractor.anthropic") as mock_anthropic:
        _configure_mock_client_with_single_replacement(mock_anthropic)
...
def _configure_mock_anthropic_client(mock_anthropic, verbose):
    mock_client = MagicMock()
    mock_anthropic.Anthropic.return_value = mock_client
    mock_anthropic.APIError = Exception
    mock_client.messages.create.side_effect = [
...
def _make_invalid_assembled_extractor(monkeypatch, verbose=True):
    ...
    with (
        ...
        return _configure_mock_anthropic_client(mock_anthropic, verbose)
...
def _configure_mock_client_with_invalid_helper(mock_anthropic):
    mock_client = MagicMock()
    mock_anthropic.Anthropic.return_value = mock_client
    mock_anthropic.APIError = Exception
    mock_client.messages.create.side_effect = [
...
def test_verify_fails_skipped(monkeypatch):
    monkeypatch.setenv("ANTHROPIC_API_KEY", "test-key")
    with patch("crispen.refactors.duplicate_extractor.anthropic") as mock_anthropic:
        _configure_mock_client_with_invalid_helper(mock_anthropic)
...
def _setup_mock_client_for_helper_extraction(mock_anthropic):
    mock_client = MagicMock()
    mock_anthropic.Anthropic.return_value = mock_client
    mock_anthropic.APIError = Exception
    mock_client.messages.create.side_effect = [
...
def test_extraction_name_collision_skipped(monkeypatch, capsys):
    ...
    monkeypatch.setenv("ANTHROPIC_API_KEY", "test-key")
    with patch("crispen.refactors.duplicate_extractor.anthropic") as mock_anthropic:
        _setup_mock_client_for_helper_extraction(mock_anthropic)
        de = DuplicateExtractor(
