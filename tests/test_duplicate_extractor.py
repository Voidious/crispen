...
def _make_extractor_with_mock_llm(mock_anthropic, verbose=False):
    mock_client = MagicMock()
    mock_anthropic.Anthropic.return_value = mock_client
    mock_anthropic.APIError = Exception
    mock_client.messages.create.side_effect = [
...
def _make_invalid_assembled_extractor(monkeypatch, verbose=True):
    ...
    with (
        ...
        return _make_extractor_with_mock_llm(mock_anthropic, verbose)
...
def _setup_steals_post_block_mock(
...
def test_replacement_steals_post_block_skipped(monkeypatch):
    ...
    monkeypatch.setenv("ANTHROPIC_API_KEY", "test-key")
    with patch("crispen.llm_client.anthropic") as mock_anthropic:
        mock_client = _setup_steals_post_block_mock(
            mock_anthropic, _make_veto_response, _make_extract_response
        )
        de = DuplicateExtractor(
            _POST_STEAL_RANGES,
...
def _setup_retry_mock_client(mock_anthropic, helper):
    mock_client = MagicMock()
    mock_anthropic.Anthropic.return_value = mock_client
    mock_anthropic.APIError = Exception
    mock_client.messages.create.side_effect = [
...
def test_extraction_retry_on_alg_failure_verbose(monkeypatch, capsys):
    ...
    helper = "def _helper(data):\n    pass\n"
    with patch("crispen.llm_client.anthropic") as mock_anthropic:
        mock_client = _setup_retry_mock_client(mock_anthropic, helper)
        de = DuplicateExtractor(
...
def _setup_mock_client_for_verify_retries(
    mock_anthropic,
    helper,
    _make_veto_response,
    _make_extract_response,
...
def test_llm_verify_rejects_then_retries_verbose(monkeypatch, capsys):
    ...
    helper = "def _helper(data):\n    pass\n"
    with patch("crispen.llm_client.anthropic") as mock_anthropic:
        mock_client = _setup_mock_client_for_verify_retries(
            mock_anthropic,
            helper,
            _make_veto_response,
            _make_extract_response,
