...
def _make_standard_extractor_setup(monkeypatch):
...
def test_successful_extraction_has_two_blank_lines(monkeypatch):
    source, helper = _make_standard_extractor_setup(monkeypatch)
...
def _setup_wrong_replacement_count_mock(mock_anthropic):
    mock_client = MagicMock()
    mock_anthropic.Anthropic.return_value = mock_client
    mock_anthropic.APIError = Exception
    mock_client.messages.create.side_effect = [
...
def test_wrong_replacement_count_skipped(monkeypatch):
    monkeypatch.setenv("ANTHROPIC_API_KEY", "test-key")
    with patch("crispen.llm_client.anthropic") as mock_anthropic:
        _setup_wrong_replacement_count_mock(mock_anthropic)
...
def _setup_mock_client_with_veto_and_extract(mock_anthropic):
    mock_client = MagicMock()
    mock_anthropic.Anthropic.return_value = mock_client
    mock_anthropic.APIError = Exception
    mock_client.messages.create.side_effect = [
...
def test_verify_fails_skipped(monkeypatch):
    monkeypatch.setenv("ANTHROPIC_API_KEY", "test-key")
    with patch("crispen.llm_client.anthropic") as mock_anthropic:
        mock_client = _setup_mock_client_with_veto_and_extract(mock_anthropic)
...
def _setup_helper_mock_client(mock_anthropic):
    mock_client = MagicMock()
    mock_anthropic.Anthropic.return_value = mock_client
    mock_anthropic.APIError = Exception
    mock_client.messages.create.side_effect = [
...
def test_extraction_name_collision_skipped(monkeypatch, capsys):
    ...
    monkeypatch.setenv("ANTHROPIC_API_KEY", "test-key")
    with patch("crispen.llm_client.anthropic") as mock_anthropic:
        mock_client = _setup_helper_mock_client(mock_anthropic)
        de = DuplicateExtractor(
