...
def _make_invalid_assembled_extractor(monkeypatch, verbose=True):
    ...
    with (
        ...
        patch(
            "crispen.refactors.duplicate_extractor.duplicate_extractor_main._apply_edits",
            return_value="def f(:\n    pass\n",  # invalid Python
        ),
    ):
        mock_client = MagicMock()
...
def _make_pyflakes_check_extractor(monkeypatch, verbose=True):
    ...
    with (
        patch("crispen.llm_client.anthropic") as mock_anthropic,
        patch(
            "crispen.refactors.duplicate_extractor.duplicate_extractor_main._pyflakes_new_undefined_names",
...
def _make_missing_free_vars_extractor(monkeypatch, verbose=True):
    ...
    with (
        patch("crispen.llm_client.anthropic") as mock_anthropic,
        patch(
            "crispen.refactors.duplicate_extractor.duplicate_extractor_main._missing_free_vars",
...
def _make_uncalled_in_combined_extractor(monkeypatch, verbose=True):
    ...
    with (
        patch("crispen.llm_client.anthropic") as mock_anthropic,
        patch(
            "crispen.refactors.duplicate_extractor.duplicate_extractor_main._has_call_to",
...
def _make_undefined_in_combined_extractor(monkeypatch, verbose=True):
    ...
    with (
        patch("crispen.llm_client.anthropic") as mock_anthropic,
        patch(
            "crispen.refactors.duplicate_extractor.duplicate_extractor_main._has_funcdef",
            side_effect=[False],
        ),
    ):
        mock_client = MagicMock()
...
def test_veto_timeout_skips_group(monkeypatch):
    ...
    with (
        ...
        patch(
            "crispen.refactors.duplicate_extractor.duplicate_extractor_main._run_with_timeout",
...
def test_func_match_verify_fails(monkeypatch):
    ...
    with (
        ...
        patch(
            "crispen.refactors.duplicate_extractor.duplicate_extractor_main._verify_extraction",
            return_value=False,
        ),
    ):
...
# ---------------------------------------------------------------------------
# _timing_out parameter: ensures timing list is populated by helper functions
# ---------------------------------------------------------------------------
