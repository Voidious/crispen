...
def _build_mock_response(block):
    resp = MagicMock()
    resp.content = [block]
    return resp
...
def _make_veto_response(is_valid: bool, reason: str = "test") -> MagicMock:
    ...
    return _build_mock_response(block)
...
def _make_naming_response(names: list[str]) -> MagicMock:
    ...
    return _build_mock_response(mock_block)
...
def _assert_splitter_skipped(ranges, src, mock_anthropic, max_lines=8):
    with patch.dict("os.environ", {"ANTHROPIC_API_KEY": "test-key"}):
...
def test_function_splitter_skip_async(mock_anthropic):
    ...
    _assert_splitter_skipped(ranges, src, mock_anthropic)
