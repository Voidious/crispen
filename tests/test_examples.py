...
def _make_mock_response(block: MagicMock) -> MagicMock:
    resp = MagicMock()
    resp.content = [block]
    return resp
...
def _make_veto_response(is_valid: bool, reason: str = "test") -> MagicMock:
    ...
    return _make_mock_response(block)
...
def _make_naming_response(names: list[str]) -> MagicMock:
    ...
    return _make_mock_response(mock_block)
...
def _assert_splitter_skipped(diff: str, src: str, mock_anthropic: MagicMock) -> None:
    ranges = _ranges(diff)
...
def test_function_splitter_skip_async(mock_anthropic):
    ...
    _assert_splitter_skipped(diff, src, mock_anthropic)
