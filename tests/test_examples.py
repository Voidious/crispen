...
def _wrap_tool_use_block(block: MagicMock) -> MagicMock:
    resp = MagicMock()
    resp.content = [block]
    return resp
...
def _make_veto_response(is_valid: bool, reason: str = "test") -> MagicMock:
    ...
    return _wrap_tool_use_block(block)
...
def _make_naming_response(names: list[str]) -> MagicMock:
    ...
    return _wrap_tool_use_block(mock_block)
...
def _setup_mock_anthropic_client(mock_anthropic, api_error=Exception):
    mock_client = MagicMock()
    mock_anthropic.Anthropic.return_value = mock_client
    if api_error is not None:
        mock_anthropic.APIError = api_error
    return mock_client
...
def test_duplicate_extraction_cross_function(monkeypatch):
    ...
    with patch("crispen.llm_client.anthropic") as mock_anthropic:
        mock_client = _setup_mock_anthropic_client(mock_anthropic)
...
def _assert_splitter_skips_llm_call(
    diff: str,
    src: str,
    ...
    *,
    ...
    with patch.dict("os.environ", {"ANTHROPIC_API_KEY": api_key}):
        splitter = FunctionSplitter(
            ranges, source=src, verbose=False, max_lines=max_lines
        )
...
def test_function_splitter_skip_async(mock_anthropic):
    ...
    _assert_splitter_skips_llm_call(diff, src, mock_anthropic, max_lines=8)
