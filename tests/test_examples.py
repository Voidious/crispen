...
def _make_response_mock(block_or_mock):
    resp = MagicMock()
    resp.content = [block_or_mock]
    return resp
...
def _make_veto_response(is_valid: bool, reason: str = "test") -> MagicMock:
    ...
    return _make_response_mock(block)
...
def _make_naming_response(names: list[str]) -> MagicMock:
    ...
    return _make_response_mock(mock_block)
...
def _run_function_splitter_with_mock(diff, src, mock_anthropic):
    ranges = _ranges(diff)
...
def test_function_splitter_skip_async(mock_anthropic):
    ...
    _run_function_splitter_with_mock(diff, src, mock_anthropic)
