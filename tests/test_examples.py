...
def _build_mock_response(block: MagicMock) -> MagicMock:
    resp = MagicMock()
    resp.content = [block]
    return resp
...
def _make_veto_response(is_valid: bool, reason: str = "test") -> MagicMock:
    ...
    resp = _build_mock_response(block)
    return resp
...
def _make_naming_response(names: list[str]) -> MagicMock:
    ...
    mock_response = _build_mock_response(mock_block)
...
def _setup_mock_anthropic_client(mock_anthropic: MagicMock) -> MagicMock:
...
def _assert_function_splitter_skips_function_kind(
    src: str, diff: str, mock_anthropic: MagicMock
) -> None:
    ranges = _ranges(diff)
...
def test_function_splitter_skip_async(mock_anthropic):
    ...
    _assert_function_splitter_skips_function_kind(src, diff, mock_anthropic)
