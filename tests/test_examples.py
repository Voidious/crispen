...
def _mock_response_with_content(block):
    resp = MagicMock()
    resp.content = [block]
    return resp
...
def _make_veto_response(is_valid: bool, reason: str = "test") -> MagicMock:
    ...
    return _mock_response_with_content(block)
...
def _make_naming_response(names: list[str]) -> MagicMock:
    ...
    return _mock_response_with_content(mock_block)
...
def _assert_no_split_occurs(diff: str, src: str, mock_anthropic) -> None:
    ranges = _ranges(diff)
...
def test_function_splitter_skip_async(mock_anthropic):
    ...
    _assert_no_split_occurs(diff, src, mock_anthropic)
