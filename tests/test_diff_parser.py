...
def _parse_and_get_ranges(diff_source, filename):
    result = parse_diff(diff_source)
...
def test_parse_simple_diff():
    ranges = _parse_and_get_ranges(SIMPLE_DIFF, "foo.py")
...
+line5
"""
    ranges = _parse_and_get_ranges(diff, "foo.py")
    assert len(ranges) == 2
    assert ranges[0] == (1, 2)
    assert ranges[1] == (4, 5)
