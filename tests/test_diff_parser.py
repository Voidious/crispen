...
def _parse_and_extract_ranges(diff_text, filename):
...
def test_parse_simple_diff():
    result, ranges = _parse_and_extract_ranges(SIMPLE_DIFF, "foo.py")
...
+line5
"""
    result, ranges = _parse_and_extract_ranges(diff, "foo.py")
    assert len(ranges) == 2
    assert ranges[0] == (1, 2)
    assert ranges[1] == (4, 5)
