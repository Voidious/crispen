...
def _parse_diff_and_get_ranges(diff_text, expected_filename="foo.py"):
    result = parse_diff(diff_text)
    assert (
        expected_filename in result
    ), f"{expected_filename} not in parse_diff result; got: {result}"
    return result[expected_filename]


def test_parse_simple_diff():
    ranges = _parse_diff_and_get_ranges(SIMPLE_DIFF)
    assert len(ranges) >= 1
    covered = set()
...
+line5
"""
    ranges = _parse_diff_and_get_ranges(diff)
    assert len(ranges) == 2
    assert ranges[0] == (1, 2)
    assert ranges[1] == (4, 5)
