...
def _parse_and_get_ranges(diff_text, expected_file="foo.py"):
    result = parse_diff(diff_text)
    assert expected_file in result
    ranges = result[expected_file]
    return result, ranges


def test_parse_simple_diff():
    result, ranges = _parse_and_get_ranges(SIMPLE_DIFF)
...
+line5
"""
    result, ranges = _parse_and_get_ranges(diff)
    assert len(ranges) == 2
    assert ranges[0] == (1, 2)
    assert ranges[1] == (4, 5)
