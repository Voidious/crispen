...
def _parse_and_extract_ranges(diff_content, filename):
    result = parse_diff(diff_content)
    assert filename in result, f"Expected {filename} in result"
    ranges = result[filename]
    return ranges


def test_parse_simple_diff():
    ranges = _parse_and_extract_ranges(SIMPLE_DIFF, "foo.py")
    assert len(ranges) >= 1
    covered = set()
...
+line5
"""
    ranges = _parse_and_extract_ranges(diff, "foo.py")
    assert len(ranges) == 2
    assert ranges[0] == (1, 2)
    assert ranges[1] == (4, 5)
