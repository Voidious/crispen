...
def _prepare_transform(source: str, ranges, min_size: int):
    if ranges is None:
        ranges = [(1, 100)]
...
def _apply(source: str, ranges=None, min_size: int = 3) -> str:
    tree, wrapper, transformer = _prepare_transform(source, ranges, min_size)
...
def _setup_tuple_dataclass(source):
    tree = cst.parse_module(source)
    from libcst.metadata import MetadataWrapper
...
def test_default_min_size_is_4_skips_3_tuple():
    ...
    source = "def _f():\n    return (1, 2, 3)\n"
    td = _setup_tuple_dataclass(source)
