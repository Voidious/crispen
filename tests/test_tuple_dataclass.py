...
def _parse_and_wrap_transformer(source: str, ranges, min_size: int):
...
def _apply(source: str, ranges=None, min_size: int = 3) -> str:
    if ranges is None:
        ranges = [(1, 100)]
    wrapper, transformer = _parse_and_wrap_transformer(source, ranges, min_size)
...
def _assert_unpacking_collector_skip(source):
    tree = cst.parse_module(source)
    collector = _UnpackingCollector()
    MetadataWrapper(tree).visit(collector)
...
def test_unpacking_collector_skips_multiple_targets():
    ...
    _assert_unpacking_collector_skip(source)
...
def _make_td_default_min_size_4(source):
    tree = cst.parse_module(source)
    from libcst.metadata import MetadataWrapper
...
def test_default_min_size_is_4_skips_3_tuple():
    ...
    source = "def _f():\n    return (1, 2, 3)\n"
    td = _make_td_default_min_size_4(source)
