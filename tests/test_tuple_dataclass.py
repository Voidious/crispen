...
def _create_transformer(ranges, source: str, min_size: int):
    if ranges is None:
        ranges = [(1, 100)]
    tree = cst.parse_module(source)
    wrapper = MetadataWrapper(tree)
    transformer = TupleDataclass(ranges, min_size=min_size, source=source)
    return tree, wrapper, transformer


def _apply(source: str, ranges=None, min_size: int = 3) -> str:
    tree, wrapper, transformer = _create_transformer(ranges, source, min_size)
...
def _assert_no_unpackings(source: str) -> None:
    tree = cst.parse_module(source)
    collector = _UnpackingCollector()
    MetadataWrapper(tree).visit(collector)
...
def _create_tuple_dataclass(source: str) -> TupleDataclass:
    tree = cst.parse_module(source)
...
def _make_td_for_public_func_3() -> TupleDataclass:
    ...
    return _create_tuple_dataclass(BEFORE_PUBLIC_FUNC_3)
...
def _make_private_td(source: str = "def _private():\n    return (1, 2, 3)\n"):
    ...
    return _create_tuple_dataclass(source)
...
def _make_td_with_default_min_size(source: str) -> TupleDataclass:
    tree = cst.parse_module(source)
    wrapper = MetadataWrapper(tree)
    td = TupleDataclass([(1, 2)], source=source)
    wrapper.visit(td)
    return td
...
def test_default_min_size_is_4_skips_3_tuple():
    ...
    source = "def _f():\n    return (1, 2, 3)\n"
    td = _make_td_with_default_min_size(source)
