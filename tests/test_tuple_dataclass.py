...
def _create_transformer(source: str, ranges, min_size: int):
    if ranges is None:
        ranges = [(1, 100)]
...
def _apply(source: str, ranges=None, min_size: int = 3) -> str:
    transformer, wrapper = _create_transformer(source, ranges, min_size)
...
def _make_tuple_dataclass(source_code: str) -> TupleDataclass:
...
def _make_td_for_public_func_3() -> TupleDataclass:
    ...
    return _make_tuple_dataclass(BEFORE_PUBLIC_FUNC_3)
...
def _make_private_td(source: str = "def _private():\n    return (1, 2, 3)\n"):
    ...
    return _make_tuple_dataclass(source)
...
def _make_td_default_min_size(source: str) -> TupleDataclass:
    tree = cst.parse_module(source)
    from libcst.metadata import MetadataWrapper
