...
def _make_td_wrapper_and_transformer(source: str, ranges, min_size: int):
    if ranges is None:
        ranges = [(1, 100)]
...
def _apply(source: str, ranges=None, min_size: int = 3) -> str:
    wrapper, transformer = _make_td_wrapper_and_transformer(source, ranges, min_size)
...
def _collect_unpackings(source: str) -> dict:
    tree = cst.parse_module(source)
    collector = _UnpackingCollector()
    MetadataWrapper(tree).visit(collector)
...
def test_unpacking_collector_skips_multiple_targets():
    ...
    assert _collect_unpackings(source) == {}
...
def _make_visited_td(source: str) -> TupleDataclass:
    tree = cst.parse_module(source)
...
def _make_td_for_public_func_3() -> TupleDataclass:
    ...
    return _make_visited_td(BEFORE_PUBLIC_FUNC_3)
...
def _make_private_td(source: str = "def _private():\n    return (1, 2, 3)\n"):
    ...
    return _make_visited_td(source)
...
def _make_td_default_min_size_visited(source: str, ranges=None):
    if ranges is None:
        ranges = [(1, 2)]
    ...
    tree = cst.parse_module(source)
    wrapper = MetadataWrapper(tree)
    td = TupleDataclass(ranges, source=source)  # no min_size → default 4
    wrapper.visit(td)
    return td
...
def test_default_min_size_is_4_skips_3_tuple():
    ...
    source = "def _f():\n    return (1, 2, 3)\n"
    td = _make_td_default_min_size_visited(source)
