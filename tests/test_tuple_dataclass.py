...
def _build_wrapper_and_transformer(source: str, ranges, min_size: int):
    if ranges is None:
        ranges = [(1, 100)]
...
def _apply(source: str, ranges=None, min_size: int = 3) -> str:
    wrapper, transformer = _build_wrapper_and_transformer(source, ranges, min_size)
...
def _make_td_from_source(source: str) -> TupleDataclass:
    tree = cst.parse_module(source)
...
def _make_td_for_public_func_3() -> TupleDataclass:
    ...
    return _make_td_from_source(BEFORE_PUBLIC_FUNC_3)
...
def _make_private_td(source: str = "def _private():\n    return (1, 2, 3)\n"):
    ...
    return _make_td_from_source(source)
...
def _build_default_min_size_transformer(source: str):
    from libcst.metadata import MetadataWrapper
...
def test_default_min_size_is_4_skips_3_tuple():
    ...
    source = "def _f():\n    return (1, 2, 3)\n"
    td = _build_default_min_size_transformer(source)
