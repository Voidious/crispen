...
def _make_transformer(source: str, ranges, min_size: int):
    if ranges is None:
        ranges = [(1, 100)]
...
def _apply(source: str, ranges=None, min_size: int = 3) -> str:
    tree, wrapper, transformer = _make_transformer(source, ranges, min_size)
...
def _parse_and_visit_td(source: str) -> TupleDataclass:
    tree = cst.parse_module(source)
...
def _make_td_for_public_func_3() -> TupleDataclass:
    ...
    td = _parse_and_visit_td(BEFORE_PUBLIC_FUNC_3)
    return td
...
def _make_td_with_default_min_size(source: str):
    tree = cst.parse_module(source)
    from libcst.metadata import MetadataWrapper
