...
def _parse_and_create_transformer(source: str, ranges=None, min_size: int = 3):
    if ranges is None:
        ranges = [(1, 100)]
...
def _apply(source: str, ranges=None, min_size: int = 3) -> str:
    wrapper, transformer = _parse_and_create_transformer(source, ranges, min_size)
...
def _check_unpacking_collector_no_unpackings(source: str) -> None:
    tree = cst.parse_module(source)
    collector = _UnpackingCollector()
    MetadataWrapper(tree).visit(collector)
...
def test_unpacking_collector_skips_multiple_targets():
    ...
    _check_unpacking_collector_no_unpackings(source)
...
def _create_tuple_dataclass_transformer(source: str):
    tree = cst.parse_module(source)
...
def _make_td_for_public_func_3() -> TupleDataclass:
    ...
    return _create_tuple_dataclass_transformer(BEFORE_PUBLIC_FUNC_3)
...
def _make_private_td(source: str = "def _private():\n    return (1, 2, 3)\n"):
    ...
    return _create_tuple_dataclass_transformer(source)
...
def _create_tuple_dataclass_transform(source):
    import libcst as cst
    from libcst.metadata import MetadataWrapper

    tree = cst.parse_module(source)
    wrapper = MetadataWrapper(tree)
    td = TupleDataclass([(1, 2)], source=source)  # no min_size
    wrapper.visit(td)
    return td
...
def test_default_min_size_is_4_skips_3_tuple():
    ...
    source = "def _f():\n    return (1, 2, 3)\n"
    td = _create_tuple_dataclass_transform(source)
