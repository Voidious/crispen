...
def _create_transformer(source: str, ranges=None, min_size: int = 3):
    if ranges is None:
        ranges = [(1, 100)]
...
def _apply(source: str, ranges=None, min_size: int = 3) -> str:
    tree, wrapper, transformer = _create_transformer(source, ranges, min_size)
...
def _assert_no_unpackings_found(source: str) -> None:
    tree = cst.parse_module(source)
    collector = _UnpackingCollector()
    MetadataWrapper(tree).visit(collector)
...
def test_unpacking_collector_skips_multiple_targets():
    ...
    _assert_no_unpackings_found(source)
...
def _create_td_from_source(source_code: str) -> TupleDataclass:
...
def _make_td_for_public_func_3() -> TupleDataclass:
    ...
    td = _create_td_from_source(BEFORE_PUBLIC_FUNC_3)
    return td
...
def _make_private_td(source: str = "def _private():\n    return (1, 2, 3)\n"):
    ...
    td = _create_td_from_source(source)
    return td
...
def _make_td_default_min_size(source: str):
    tree = cst.parse_module(source)
    from libcst.metadata import MetadataWrapper
...
def test_default_min_size_is_4_skips_3_tuple():
    ...
    source = "def _f():\n    return (1, 2, 3)\n"
    td = _make_td_default_min_size(source)
