...
def _make_td_helper(source: str, ranges, min_size: int):
    if ranges is None:
        ranges = [(1, 100)]
...
def _apply(source: str, ranges=None, min_size: int = 3) -> str:
    wrapper, transformer = _make_td_helper(source, ranges, min_size)
...
def _make_default_min_size_td(source):
    tree = cst.parse_module(source)
    wrapper = MetadataWrapper(tree)
...
def test_default_min_size_is_4_skips_3_tuple():
    ...
    source = "def _f():\n    return (1, 2, 3)\n"
    td = _make_default_min_size_td(source)
