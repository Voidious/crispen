...
def _run_unpacking_collector_and_assert_empty(source: str) -> None:
    tree = cst.parse_module(source)
    collector = _UnpackingCollector()
    MetadataWrapper(tree).visit(collector)
...
def test_unpacking_collector_skips_multiple_targets():
    ...
    _run_unpacking_collector_and_assert_empty(source)
...
def _make_td(source: str) -> TupleDataclass:
    tree = cst.parse_module(source)
...
def _make_td_for_public_func_3() -> TupleDataclass:
    ...
    return _make_td(BEFORE_PUBLIC_FUNC_3)
...
def visit_tuple_dataclass_for_source(source):
    tree = cst.parse_module(source)
    from libcst.metadata import MetadataWrapper
...
def test_default_min_size_is_4_skips_3_tuple():
    ...
    source = "def _f():\n    return (1, 2, 3)\n"
    td = visit_tuple_dataclass_for_source(source)
