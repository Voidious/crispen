...
def _prepare_transformer(source: str, ranges, min_size: int):
    if ranges is None:
        ranges = [(1, 100)]
    tree = cst.parse_module(source)
    wrapper = MetadataWrapper(tree)
    transformer = TupleDataclass(ranges, min_size=min_size, source=source)
    return transformer, wrapper, tree


def _apply(source: str, ranges=None, min_size: int = 3) -> str:
    transformer, wrapper, tree = _prepare_transformer(source, ranges, min_size)
...
def test_unpacking_collector_skips_multiple_targets():
    ...
    collector = _collect_unpackings(source)
...
def _make_td_for_public_func_3() -> TupleDataclass:
    ...
    return _create_td_from_source(BEFORE_PUBLIC_FUNC_3)
...
def _make_private_td(source: str = "def _private():\n    return (1, 2, 3)\n"):
    ...
    return _create_td_from_source(source)
...
def _prepare_tuple_dataclass(source):
    import libcst as cst
    from libcst.metadata import MetadataWrapper
...
def test_default_min_size_is_4_skips_3_tuple():
    ...
    source = "def _f():\n    return (1, 2, 3)\n"
    td = _prepare_tuple_dataclass(source)
