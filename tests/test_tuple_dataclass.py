...
def _apply(source: str, ranges=None, min_size: int = 3) -> str:
    transformer, wrapper = _prepare_transformer(source, ranges, min_size)
...
def _create_and_visit_td(source: str, ranges=None):
    tree = cst.parse_module(source)
    from libcst.metadata import MetadataWrapper

    wrapper = MetadataWrapper(tree)
    td = TupleDataclass(ranges if ranges is not None else [(1, 2)], source=source)
    wrapper.visit(td)
    return td
...
def test_default_min_size_is_4_skips_3_tuple():
    ...
    source = "def _f():\n    return (1, 2, 3)\n"
    td = _create_and_visit_td(source, [(1, 2)])
