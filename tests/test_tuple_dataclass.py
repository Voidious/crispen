...
def _check_no_unpackings(source: str) -> None:
    tree = cst.parse_module(source)
    collector = _UnpackingCollector()
    MetadataWrapper(tree).visit(collector)
...
def _build_transformer(source: str) -> TupleDataclass:
    tree = cst.parse_module(source)
...
def _make_td_for_public_func_3() -> TupleDataclass:
    ...
    td = _build_transformer(BEFORE_PUBLIC_FUNC_3)
    return td
