...
def _run_unpacking_collector(source: str) -> _UnpackingCollector:
    tree = cst.parse_module(source)
    collector = _UnpackingCollector()
    MetadataWrapper(tree).visit(collector)
    return collector
...
def test_unpacking_collector_skips_multiple_targets():
    ...
    collector = _run_unpacking_collector(source)
...
def _create_and_visit_td(source: str) -> TupleDataclass:
    tree = cst.parse_module(source)
...
def _make_td_for_public_func_3() -> TupleDataclass:
    ...
    return _create_and_visit_td(BEFORE_PUBLIC_FUNC_3)
...
def _make_private_td(source: str = "def _private():\n    return (1, 2, 3)\n"):
    ...
    return _create_and_visit_td(source)
...
def _make_td_no_min_size(source):
    tree = cst.parse_module(source)
    from libcst.metadata import MetadataWrapper
...
def test_default_min_size_is_4_skips_3_tuple():
    ...
    source = "def _f():\n    return (1, 2, 3)\n"
    td = _make_td_no_min_size(source)
