...
def _llm_helper_expected_names(mock_anthropic, mock_response):
    mock_anthropic.Anthropic.return_value.messages.create.return_value = mock_response
    tasks = [_make_task("my_func")]
    client = mock_anthropic.Anthropic.return_value
...
def test_llm_name_helpers_result_none(mock_anthropic):
    ...
    result = _llm_helper_expected_names(mock_anthropic, mock_response)
...
def _run_splitter_expect_no_rewrite(
    src: str, ranges=None, verbose: bool = False, max_lines: int = 10
):
    if ranges is None:
        ranges = [(1, 1000)]
    with patch.dict("os.environ", {"ANTHROPIC_API_KEY": "test-key"}):
...
def test_function_splitter_nested_funcdef_not_split(mock_anthropic):
    ...
    _run_splitter_expect_no_rewrite(src)
...
def test_function_splitter_pyflakes_rejects_output(mock_anthropic, mock_has_undef):
    ...
    splitter = _make_splitter_no_rewrite(src)
