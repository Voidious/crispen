...
def _check_no_functions_collected(src: str):
    tree = cst.parse_module(src)
    wrapper = MetadataWrapper(tree)
    collector = _FunctionCollector()
    wrapper.visit(collector)
...
def test_function_collector_skips_async():
    src = "async def foo():\n    pass\n"
    _check_no_functions_collected(src)
...
def test_function_collector_skips_nested_functions():
    ...
    _check_no_functions_collected(
        src
    )  # outer has a nested funcdef 1 skipped; inner is in a function scope 1 skipped
...
def _setup_llm_name_helpers_call(mock_anthropic, mock_response, task_name):
    mock_anthropic.Anthropic.return_value.messages.create.return_value = mock_response
...
def test_llm_name_helpers_result_none(mock_anthropic):
    ...
    result = _setup_llm_name_helpers_call(mock_anthropic, mock_response, "my_func")
