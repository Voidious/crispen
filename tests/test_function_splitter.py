...
def _make_splitter_with_api_key(src: str) -> FunctionSplitter:
    with patch.dict("os.environ", {"ANTHROPIC_API_KEY": "test-key"}):
        return FunctionSplitter([(1, 1000)], source=src, verbose=False, max_lines=10)
...
def test_function_splitter_nested_funcdef_not_split(mock_anthropic):
    ...
    splitter = _make_splitter_with_api_key(src)
