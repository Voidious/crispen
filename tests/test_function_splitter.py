...
def _assert_no_collected_functions(src: str) -> None:
    tree = cst.parse_module(src)
    wrapper = MetadataWrapper(tree)
    collector = _FunctionCollector()
    wrapper.visit(collector)
...
def test_function_collector_skips_async():
    src = "async def foo():\n    pass\n"
    _assert_no_collected_functions(src)
...
def _llm_name_helpers_common_call(mock_anthropic, mock_response):
    mock_anthropic.Anthropic.return_value.messages.create.return_value = mock_response
...
def test_llm_name_helpers_result_none(mock_anthropic):
    ...
    result = _llm_name_helpers_common_call(mock_anthropic, mock_response)
...
def _function_splitter_common_no_rewrite_helper(src: str):
    with patch.dict("os.environ", {"ANTHROPIC_API_KEY": "test-key"}):
        splitter = FunctionSplitter(
...
def test_function_splitter_nested_funcdef_not_split(mock_anthropic):
    ...
    _function_splitter_common_no_rewrite_helper(src)
...
def _function_splitter_common_no_rewrite(src):
    with patch.dict("os.environ", {"ANTHROPIC_API_KEY": "test-key"}):
        splitter = FunctionSplitter(
            [(1, 1000)], source=src, verbose=False, max_lines=50
        )
...
def test_function_splitter_pyflakes_rejects_output(mock_anthropic, mock_has_undef):
    ...
    splitter = _function_splitter_common_no_rewrite(src)
