...
def _assert_splitter_skips_source(src: str) -> None:
    with patch.dict("os.environ", {"ANTHROPIC_API_KEY": "test-key"}):
        splitter = FunctionSplitter(
...
def test_function_splitter_nested_funcdef_not_split(mock_anthropic):
    ...
    _assert_splitter_skips_source(src)
...
def _create_splitter_for_test(src: str, max_lines: int = 50) -> FunctionSplitter:
    with patch.dict("os.environ", {"ANTHROPIC_API_KEY": "test-key"}):
        splitter = FunctionSplitter(
...
def test_function_splitter_pyflakes_rejects_output(mock_anthropic, mock_has_undef):
    ...
    splitter = _create_splitter_for_test(src)
