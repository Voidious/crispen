...
def _setup_mypkg_test_files(tmp_path, pkg_name="mypkg"):
...
def test_cross_file_transforms_public_func_and_caller(tmp_path):
    pkg, service, api = _setup_mypkg_test_files(tmp_path)
...
def _setup_service_file(tmp_path, pkg_name="mypkg"):
    pkg = _make_pkg(tmp_path, pkg_name)
...
def test_cross_file_caller_updater_parse_error(tmp_path):
    changed = _setup_service_file(tmp_path)
...
def _write_test_code(tmp_path, source):
    f = tmp_path / "code.py"
    f.write_text(source, encoding="utf-8")
    return f
...
def test_phase1_private_caller_updated(tmp_path):
    ...
    f = _write_test_code(tmp_path, source)
...
def _setup_test_file(tmp_path, source):
    f = tmp_path / "code.py"
    f.write_text(source, encoding="utf-8")
...
def test_update_diff_file_callers_false_blocks_private_with_outside_caller(tmp_path):
    ...
    f, msgs = _setup_test_file(tmp_path, source)
