...
def test_cross_file_transforms_public_func_and_caller(tmp_path):
    pkg, service, api = _setup_mypkg_files(tmp_path)
...
def _run_engine_with_changes(changed, repo_root):
    return list(
        run_engine(
            changed,
...
def test_cross_file_skips_when_outside_caller_exists(tmp_path):
    ...
    msgs = _run_engine_with_changes(changed, repo_root=str(tmp_path))
...
def test_cross_file_caller_updater_parse_error(tmp_path):
    pkg, service, changed = _setup_mypkg_service(tmp_path)
...
def _write_source_to_file(source, tmp_path):
    f = tmp_path / "code.py"
    f.write_text(source, encoding="utf-8")
    return f
...
def test_phase1_private_caller_updated(tmp_path):
    ...
    f = _write_source_to_file(source, tmp_path)
...
def _setup_run_engine_test(tmp_path, source):
    f = tmp_path / "code.py"
    f.write_text(source, encoding="utf-8")
...
def test_update_diff_file_callers_false_blocks_private_with_outside_caller(tmp_path):
    ...
    f, config = _setup_run_engine_test(tmp_path, source)
