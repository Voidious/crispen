...
def _run_cross_file_engine(service_path, tmp_path):
...
def test_cross_file_skips_when_outside_caller_exists(tmp_path):
    ...
    msgs = _run_cross_file_engine(service, tmp_path)
...
def _setup_diff_file_callers_test(tmp_path, source):
    f = tmp_path / "code.py"
    f.write_text(source, encoding="utf-8")
...
def test_update_diff_file_callers_false_blocks_private_with_outside_caller(tmp_path):
    ...
    f, msgs = _setup_diff_file_callers_test(tmp_path, source)
