...
def _setup_pkg_service_api(tmp_path, pkg_name="mypkg"):
...
def test_cross_file_transforms_public_func_and_caller(tmp_path):
    pkg, service, api = _setup_pkg_service_api(tmp_path)
...
def _setup_service_file(tmp_path, pkg_name):
    pkg = _make_pkg(tmp_path, pkg_name)
    service = pkg / "service.py"
...
def test_cross_file_caller_updater_parse_error(tmp_path):
    changed = _setup_service_file(tmp_path, "mypkg")
