...
def _setup_cross_file_test_pkg(tmp_path):
...
def test_cross_file_transforms_public_func_and_caller(tmp_path):
    pkg, service, api = _setup_cross_file_test_pkg(tmp_path)
...
def _run_engine_cross_file_test(changed, tmp_path):
    msgs = list(
        run_engine(
            changed,
            _repo_root=str(tmp_path),
...
def test_cross_file_skips_when_outside_caller_exists(tmp_path):
    ...
    msgs = _run_engine_cross_file_test(changed, tmp_path)
...
def _write_phase1_test_file(tmp_path):
    source = textwrap.dedent(
        """\
        def _make_result():
            return (1, 2, 3)
...
def test_phase1_private_caller_updated(tmp_path):
    ...
    f = _write_phase1_test_file(tmp_path)
...
def setup_test_file_and_run(tmp_path, source, line_range=(1, 5)):
    f = tmp_path / "code.py"
    f.write_text(source, encoding="utf-8")
...
def test_update_diff_file_callers_false_blocks_private_with_outside_caller(tmp_path):
    ...
    f, msgs = setup_test_file_and_run(tmp_path, source)
