...
def _setup_mypkg_service_api(tmp_path):
...
def test_cross_file_transforms_public_func_and_caller(tmp_path):
    pkg, service, api = _setup_mypkg_service_api(tmp_path)
...
def _run_engine_with_outside_caller_check(tmp_path, service_file):
    changed = {str(service_file): [(1, 2)]}
    msgs = list(
        run_engine(
            changed,
            _repo_root=str(tmp_path),
...
def test_cross_file_skips_when_outside_caller_exists(tmp_path):
    ...
    _run_engine_with_outside_caller_check(tmp_path, service)
...
def _setup_mypkg_service(tmp_path):
    pkg = _make_pkg(tmp_path, "mypkg")
...
def test_cross_file_caller_updater_parse_error(tmp_path):
    changed = _setup_mypkg_service(tmp_path)
...
def _prepare_file_and_run_engine(tmp_path, source):
    f = tmp_path / "code.py"
    f.write_text(source, encoding="utf-8")
...
def test_update_diff_file_callers_false_blocks_private_with_outside_caller(tmp_path):
    ...
    f, msgs = _prepare_file_and_run_engine(tmp_path, source)
