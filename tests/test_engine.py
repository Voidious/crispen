...
def _make_mypkg_service_and_api(tmp_path):
...
def test_cross_file_transforms_public_func_and_caller(tmp_path):
    pkg, service, api = _make_mypkg_service_and_api(tmp_path)
...
def test_cross_file_caller_updater_parse_error(tmp_path):
    pkg, service, changed = _make_service_changed(tmp_path)
...
def _run_with_diff_file_callers_false(tmp_path, source):
    f = tmp_path / "code.py"
    f.write_text(source, encoding="utf-8")
...
def test_update_diff_file_callers_false_blocks_private_with_outside_caller(tmp_path):
    ...
    f, msgs = _run_with_diff_file_callers_false(tmp_path, source)
