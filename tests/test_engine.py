...
def _write_service_and_api_get_user(tmp_path):
...
def test_cross_file_transforms_public_func_and_caller(tmp_path):
    service, api = _write_service_and_api_get_user(tmp_path)
...
def _run_cross_file_outside_caller_blocked_scenario(service, tmp_path):
...
def test_cross_file_skips_when_outside_caller_exists(tmp_path):
    ...
    _run_cross_file_outside_caller_blocked_scenario(service, tmp_path)
...
def _write_private_caller_sample(tmp_path):
    source = textwrap.dedent(
        """\
        def _make_result():
            return (1, 2, 3)
...
def test_phase1_private_caller_updated(tmp_path):
    ...
    f = _write_private_caller_sample(tmp_path)
...
def _write_code_and_run_engine_min_tuple_3_update_diff_file_callers_false(
    tmp_path, source
):
    f = tmp_path / "code.py"
    f.write_text(source, encoding="utf-8")
...
def test_update_diff_file_callers_false_blocks_private_with_outside_caller(tmp_path):
    ...
    f, msgs = _write_code_and_run_engine_min_tuple_3_update_diff_file_callers_false(
        tmp_path, source
    )
