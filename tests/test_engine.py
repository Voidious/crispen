...
def _setup_cross_file_pkg(tmp_path, pkg_name="mypkg"):
...
def test_cross_file_transforms_public_func_and_caller(tmp_path):
    pkg, service, api = _setup_cross_file_pkg(tmp_path, "mypkg")
...
def _run_engine_and_assert_outside_caller(changed, _repo_root, config):
    msgs = list(run_engine(changed, _repo_root=_repo_root, config=config))
...
def test_cross_file_skips_when_outside_caller_exists(tmp_path):
    ...
    msgs = _run_engine_and_assert_outside_caller(
        {str(service): [(1, 2)]},
        _repo_root=str(tmp_path),
        config=CrispenConfig(min_tuple_size=3),
    )
...
def _setup_cross_file_service(
    tmp_path,
    service_content="def approved():\n    return (1, 2, 3)\n",
    changed_ranges=None,
):
    ...
    changed = {str(service): changed_ranges}
    return pkg, service, changed


def test_cross_file_caller_updater_parse_error(tmp_path):
    pkg, service, changed = _setup_cross_file_service(tmp_path)
...
def _write_test_file(tmp_path, source):
    f = tmp_path / "code.py"
    f.write_text(source, encoding="utf-8")
    return f
...
def test_phase1_private_caller_updated(tmp_path):
    ...
    f = _write_test_file(tmp_path, source)
...
def _setup_and_run_engine(tmp_path, source, line_range, config):
    f = tmp_path / "code.py"
    f.write_text(source, encoding="utf-8")
    msgs = list(run_engine({str(f): [line_range]}, config=config))
    return f, msgs
...
def test_update_diff_file_callers_false_blocks_private_with_outside_caller(tmp_path):
    ...
    f, msgs = _setup_and_run_engine(
        tmp_path,
        source,
        (1, 5),
        CrispenConfig(min_tuple_size=3, update_diff_file_callers=False),
    )
