...
def make_mypkg_with_service_and_api(tmp_path):
...
def test_cross_file_transforms_public_func_and_caller(tmp_path):
    service, api = make_mypkg_with_service_and_api(tmp_path)
...
def _assert_outside_callers_exist(service, tmp_path):
...
def _write_sample_code_make_result(tmp_path):
    source = textwrap.dedent(
        """\
        def _make_result():
            return (1, 2, 3)
...
def test_phase1_private_caller_updated(tmp_path):
    ...
    f = _write_sample_code_make_result(tmp_path)
