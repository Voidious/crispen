...
def _make_cross_file_test_pkg(tmp_path):
...
def test_cross_file_transforms_public_func_and_caller(tmp_path):
    pkg, service, api = _make_cross_file_test_pkg(tmp_path)
...
def _run_cross_file_engine(tmp_path, service_path):
...
def test_cross_file_skips_when_outside_caller_exists(tmp_path):
    ...
    _run_cross_file_engine(tmp_path, service)
...
def test_cross_file_caller_updater_parse_error(tmp_path):
    pkg, service, changed = _setup_cross_file_test_pkg(tmp_path)
...
def _create_phase1_test_file(tmp_path):
    source = textwrap.dedent(
        """\
        def _make_result():
            return (1, 2, 3)
...
def test_phase1_private_caller_updated(tmp_path):
    ...
    f = _create_phase1_test_file(tmp_path)
