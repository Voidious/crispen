...
def run_with_patched_refactors(tmp_path, refactor_cls):
    f = tmp_path / "code.py"
    f.write_text("x = 1\n", encoding="utf-8")
...
def test_rewritten_source_used_when_available(tmp_path):
    ...
    f, msgs = run_with_patched_refactors(tmp_path, _RewritingRefactor)
...
def test_skip_transform_error(tmp_path):
    f, msgs = run_with_patched_refactors(tmp_path, _RaisingTransformer)
