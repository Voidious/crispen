...
def test_tuple_dataclass_transform_error_handled(tmp_path):
    ...
    with patch("crispen.engine.helpers.TupleDataclass", _FailingTD):
        msgs = _run({str(f): [(1, 1)]})
...
def test_find_outside_callers_manager_build_fails(tmp_path):
    ...
    with patch(
        "crispen.engine.helpers.FullRepoManager", side_effect=RuntimeError("fail")
    ):
...
def test_find_outside_callers_wrapper_fails(tmp_path):
    (tmp_path / "other.py").write_text("x = 1\n")
    with patch("crispen.engine.helpers.FullRepoManager") as MockFRM:
...
def test_build_patch_map_new_module_none(tmp_path):
    ...
    with patch(
        "crispen.engine.file_limiter_import_redirect._module_path_for_file",
