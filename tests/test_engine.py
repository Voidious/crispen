...
def test_tuple_dataclass_transform_error_handled(tmp_path):
    ...
    with patch("crispen.engine.utils.TupleDataclass", _FailingTD):
        msgs = _run({str(f): [(1, 1)]})
...
def test_find_outside_callers_manager_build_fails(tmp_path):
    ...
    with patch(
        "crispen.engine.utils.FullRepoManager", side_effect=RuntimeError("fail")
    ):
...
def test_find_outside_callers_wrapper_fails(tmp_path):
    (tmp_path / "other.py").write_text("x = 1\n")
    with patch("crispen.engine.utils.FullRepoManager") as MockFRM:
...
def test_apply_tuple_dataclass_crispen_api_error():
    with patch("crispen.engine.utils.MetadataWrapper") as MockWrapper:
