...
def _setup_pkg_with_service_api(tmp_path):
...
def test_cross_file_transforms_public_func_and_caller(tmp_path):
    pkg, service, api = _setup_pkg_with_service_api(tmp_path)
...
def _run_engine_with_root(changed, tmp_path, min_tuple_size=3):
    msgs = list(
        run_engine(
            changed,
            _repo_root=str(tmp_path),
...
def test_cross_file_skips_when_outside_caller_exists(tmp_path):
    ...
    msgs = _run_engine_with_root(changed, tmp_path)
...
def _setup_service(tmp_path, pkg_name="mypkg"):
...
def test_cross_file_caller_updater_parse_error(tmp_path):
    pkg, service, changed = _setup_service(tmp_path)
...
def test_phase1_private_caller_updated(tmp_path):
    ...
    f = _write_code_file(tmp_path, source)
...
def _setup_no_update_diff(tmp_path, source):
    f = tmp_path / "code.py"
    f.write_text(source, encoding="utf-8")
...
def test_update_diff_file_callers_false_blocks_private_with_outside_caller(tmp_path):
    ...
    f, msgs = _setup_no_update_diff(tmp_path, source)
