...
def _make_service_and_api(tmp_path, pkg):
    service = pkg / "service.py"
    service.write_text(
...
def test_cross_file_transforms_public_func_and_caller(tmp_path):
    ...
    service, api = _make_service_and_api(tmp_path, pkg)
...
def test_cross_file_caller_updater_parse_error(tmp_path):
    changed = _make_service_pkg_and_changed(tmp_path)
...
def _make_private_caller_source_file(tmp_path):
    source = textwrap.dedent(
        """\
        def _make_result():
            return (1, 2, 3)
...
def test_phase1_private_caller_updated(tmp_path):
    ...
    f = _make_private_caller_source_file(tmp_path)
...
def _run_with_no_diff_callers(tmp_path, source):
    f = tmp_path / "code.py"
    f.write_text(source, encoding="utf-8")
...
def test_update_diff_file_callers_false_blocks_private_with_outside_caller(tmp_path):
    ...
    f, msgs = _run_with_no_diff_callers(tmp_path, source)
