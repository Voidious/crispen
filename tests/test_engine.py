...
def _make_pkg_with_service_api(tmp_path):
...
def test_cross_file_transforms_public_func_and_caller(tmp_path):
    pkg, service, api = _make_pkg_with_service_api(tmp_path)
...
def _run_cross_file_test(service, tmp_path):
...
def test_cross_file_skips_when_outside_caller_exists(tmp_path):
    ...
    _run_cross_file_test(service, tmp_path)
...
def _setup_cross_file_service_pkg(tmp_path):
    pkg = _make_pkg(tmp_path, "mypkg")
...
def test_cross_file_caller_updater_parse_error(tmp_path):
    pkg, changed = _setup_cross_file_service_pkg(tmp_path)
...
def create_test_code_file(tmp_path):
    source = textwrap.dedent(
        """\
        def _make_result():
            return (1, 2, 3)
...
def test_phase1_private_caller_updated(tmp_path):
    ...
    f = create_test_code_file(tmp_path)
...
def _setup_and_run_engine(tmp_path, source, ranges, update_diff_file_callers=False):
    f = tmp_path / "code.py"
    f.write_text(source, encoding="utf-8")
    config = CrispenConfig(
        min_tuple_size=3, update_diff_file_callers=update_diff_file_callers
    )
    msgs = list(run_engine({str(f): ranges}, config=config))
    return f, msgs
...
def test_update_diff_file_callers_false_blocks_private_with_outside_caller(tmp_path):
    ...
    f, msgs = _setup_and_run_engine(
        tmp_path, source, [(1, 5)], update_diff_file_callers=False
    )
