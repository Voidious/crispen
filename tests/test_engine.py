...
def _setup_service_api_pkg(tmp_path, pkg_name="mypkg"):
...
def test_cross_file_transforms_public_func_and_caller(tmp_path):
    pkg, service, api = _setup_service_api_pkg(tmp_path)
...
def test_cross_file_skips_when_outside_caller_exists(tmp_path):
    ...
    _run_cross_file_test(tmp_path, service)
...
def _setup_and_run_engine(tmp_path, source, update_diff_file_callers=False):
    f = tmp_path / "code.py"
    f.write_text(source, encoding="utf-8")
    config = CrispenConfig(
