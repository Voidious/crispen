...
def _run_cross_file_test(changed, tmp_path):
    msgs = list(
        run_engine(
            changed,
            _repo_root=str(tmp_path),
...
def test_cross_file_skips_when_outside_caller_exists(tmp_path):
    ...
    msgs = _run_cross_file_test(changed, tmp_path)
...
def _write_phase1_source(tmp_path, filename="code.py"):
    source = textwrap.dedent(
        """\
        def _make_result():
            return (1, 2, 3)
...
def _setup_and_run_engine(tmp_path, source, ranges, *, comment=None):
    f = tmp_path / "code.py"
    f.write_text(source, encoding="utf-8")
    config = CrispenConfig(min_tuple_size=3, update_diff_file_callers=False)
    if comment is not None:
        pass  # comment is test-specific, kept for API compatibility
...
def test_update_diff_file_callers_false_blocks_private_with_outside_caller(tmp_path):
    ...
    f, msgs = _setup_and_run_engine(tmp_path, source, [(1, 5)])
