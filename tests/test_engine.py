...
def test_cross_file_transforms_public_func_and_caller(tmp_path):
    api, service = _make_user_pkg(tmp_path)
...
def _run_outside_caller_check(service, tmp_path, min_tuple_size=3):
    changed = {str(service): [(1, 2)]}
    msgs = list(
        run_engine(
            changed,
            _repo_root=str(tmp_path),
            config=CrispenConfig(min_tuple_size=min_tuple_size),
        )
    )
...
def test_cross_file_skips_when_outside_caller_exists(tmp_path):
    ...
    _run_outside_caller_check(service, tmp_path)
...
def _make_service_pkg_and_changed(tmp_path):
    pkg = _make_pkg(tmp_path, "mypkg")
...
def test_cross_file_caller_updater_parse_error(tmp_path):
    pkg, service, changed = _make_service_pkg_and_changed(tmp_path)
...
def _write_phase1_sample_code(tmp_path):
    source = textwrap.dedent(
        """\
        def _make_result():
            return (1, 2, 3)
...
def test_phase1_private_caller_updated(tmp_path):
    ...
    f = _write_phase1_sample_code(tmp_path)
...
def _make_diff_run_with_update_diff_file_callers_false(
    tmp_path, source, min_tuple_size=3, update_diff_file_callers=False, diff_ranges=None
):
    ...
    f.write_text(source, encoding="utf-8")
    config = CrispenConfig(
        min_tuple_size=min_tuple_size, update_diff_file_callers=update_diff_file_callers
    )
    msgs = list(run_engine({str(f): diff_ranges}, config=config))
    return f, msgs
...
def test_update_diff_file_callers_false_blocks_private_with_outside_caller(tmp_path):
    ...
    f, msgs = _make_diff_run_with_update_diff_file_callers_false(tmp_path, source)
