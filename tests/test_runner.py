...
def _create_split_and_verify(entity, post_source):
    split = SplitResult(
        new_files={},
...
def test_verify_entity_source_missing():
    post_source = "def foo():\n    pass\n"
    entity = _make_entity("foo", 1, 2)
    failures = _create_split_and_verify(entity, post_source)
...
def _run_limiter_with_retries(mock_gen, mock_advise):
...
def test_runner_plan_abort_retries_and_succeeds(mock_classify, mock_advise, mock_gen):
    ...
    result = _run_limiter_with_retries(mock_gen, mock_advise)
...
def _run_file_limiter_with_mock(mock_classify, mock_advise, classified_return, config):
    mock_classify.return_value = classified_return
...
def test_runner_no_placements(mock_classify, mock_advise):
    result, source = _run_file_limiter_with_mock(
...
def _prepare_mock_classify_advise_gen(mock_classify, mock_advise, mock_gen):
    source = "def foo():\n    pass\n"
    entity = _make_entity("foo", 1, 2)
    mock_classify.return_value = _make_classified(entities=[entity])
    mock_advise.return_value = _plan_with(["foo"], "utils.py")
    ...
    return mock_advise, mock_gen, source
...
def test_runner_split_aborts_with_reason(mock_classify, mock_advise, mock_gen):
    mock_advise, mock_gen, source = _prepare_mock_classify_advise_gen(
...
def _run_file_limiter_with_generated_split(mock_gen, source):
    mock_gen.return_value = SplitResult(
...
def test_runner_adds_test_prefix_to_new_files(mock_classify, mock_advise, mock_gen):
    ...
    result = _run_file_limiter_with_generated_split(mock_gen, source)
    assert not any(
