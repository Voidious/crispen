...
def _make_openai_response(tool_name: str, arguments: dict) -> MagicMock:
    ...
    resp = _create_mock_response(tc)
    return resp


def _setup_mock_openai_client(mock_oai, _make_openai_response):
...
def test_call_with_tool_moonshot_success():
    with patch("crispen.llm_client.openai") as mock_oai:
        client = _setup_mock_openai_client(mock_oai, _make_openai_response)
        result = call_with_tool(
            client,
            "moonshot",
            "moonshot-v1-32k",
...
def _setup_moonshot_openai_api_error_mock(mock_oai):
    mock_oai.APIError = Exception
    client = MagicMock()
...
def test_call_with_tool_moonshot_api_error():
    with patch("crispen.llm_client.openai") as mock_oai:
        client = _setup_moonshot_openai_api_error_mock(mock_oai)
