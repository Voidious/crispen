...
def _make_mock_openai_tool_call_response(tc):
    message = MagicMock()
...
def _make_openai_response(tool_name: str, arguments: dict) -> MagicMock:
    ...
    resp = _make_mock_openai_tool_call_response(tc)
    return resp


def _setup_mock_openai_client_with_tool_response(mock_oai, tool_name, tool_args):
...
def test_call_with_tool_moonshot_success():
    with patch("crispen.llm_client.openai") as mock_oai:
        client = _setup_mock_openai_client_with_tool_response(
            mock_oai,
            "evaluate_duplicate",
            {"is_valid_duplicate": True, "reason": "same"},
        )
        result = call_with_tool(
            client,
            "moonshot",
...
def _setup_mock_openai_rate_limit_error(mock_oai):
    mock_oai.APIError = Exception
    client = MagicMock()
...
def test_call_with_tool_moonshot_api_error():
    with patch("crispen.llm_client.openai") as mock_oai:
        client = _setup_mock_openai_rate_limit_error(mock_oai)
