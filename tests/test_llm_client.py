...
def _build_openai_tool_response(tc):
    message = MagicMock()
...
def _make_openai_response(tool_name: str, arguments: dict) -> MagicMock:
    ...
    resp = _build_openai_tool_response(tc)
    return resp


def _make_mock_client_with_successful_tool_call(mock_oai):
...
def test_call_with_tool_moonshot_success():
    with patch("crispen.llm_client.openai") as mock_oai:
        client = _make_mock_client_with_successful_tool_call(mock_oai)
        result = call_with_tool(
            client,
            "moonshot",
            "moonshot-v1-32k",
...
def _configure_mock_openai_rate_limit_error(mock_oai):
    mock_oai.APIError = Exception
    client = MagicMock()
...
def test_call_with_tool_moonshot_api_error():
    with patch("crispen.llm_client.openai") as mock_oai:
        client = _configure_mock_openai_rate_limit_error(mock_oai)
