...
def _setup_openai_mock_client(mock_oai):
...
def test_call_with_tool_moonshot_success():
    with patch("crispen.llm_client.openai") as mock_oai:
        client = _setup_openai_mock_client(mock_oai)
        result = call_with_tool(
            client,
            "moonshot",
            "moonshot-v1-32k",
...
def _setup_openai_api_error_client(mock_oai):
    mock_oai.APIError = Exception
    client = MagicMock()
...
def test_call_with_tool_moonshot_api_error():
    with patch("crispen.llm_client.openai") as mock_oai:
        client = _setup_openai_api_error_client(mock_oai)
