...
def _assert_import_only_block_unmodified(
    post: str, block_name: str = "_block_1"
) -> None:
    result = classify_entities(post, post, [(1, 2)])
    assert result.entity_class[block_name] == EntityClass.UNMODIFIED, block_name
    assert block_name in result.set_1, block_name
...
def test_classify_import_only_block_stays_unmodified_even_when_diff_overlaps():
    ...
    _assert_import_only_block_unmodified(post)
