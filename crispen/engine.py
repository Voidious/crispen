...
def _apply_changes_and_update_source(
    change_provider, filepath: str, updated_source: str, file_msgs: List[str]
) -> str:
    for msg in change_provider.get_changes():
...
def run_engine(
    ...
    for filepath, ranges in changed.items():
        ...
        for RefactorClass in _REFACTORS:
            ...
            current_source = _apply_changes_and_update_source(
                transformer, filepath, new_source, file_msgs
            )
                ...
                if private_transforms:
                    ...
                    if cu_new_source != current_source:
                        try:
                            ...
                            current_source = _apply_changes_and_update_source(
                                cu, filepath, cu_new_source, file_msgs
                            )
