...
def _ensure_trailing_newline(lines: list) -> list:
    """
    Ensure the last element of a splitlines(keepends=True) list ends with a newline.
    """
...
def _apply_edits(source: str, edits: List[Tuple[int, int, str]]) -> str:
    ...
    """
    lines = _ensure_trailing_newline(source.splitlines(keepends=True))
    ...
    for start, end, text in sorted(edits, key=lambda e: (e[0], e[1]), reverse=True):
        ...
        new_lines = _ensure_trailing_newline(text.splitlines(keepends=True))
