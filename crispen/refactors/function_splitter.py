...
def _maybe_update_best_split(current_idx, current_params, best_idx, best_params):
    ...
    * ``is_instance_method=True`` — the tail references ``self``, so the
    ...
    for split_idx in valid_splits:
        ...
        if is_method and "self" in free:
            ...
            best_instance_idx, best_instance_params = _maybe_update_best_split(
                split_idx, params_no_self, best_instance_idx, best_instance_params
            )
        else:
            best_static_idx, best_static_params = _maybe_update_best_split(
