...
def _update_best_candidate(
    ...
    if current_best_idx is None or len(candidate_params) < len(current_best_params):
        return candidate_idx, candidate_params
    return current_best_idx, current_best_params
...
class FunctionSplitter(Refactor):
    ...
    def _split_functions(self, source):
        current = source
        return self._split_large_functions(current, source)

    def _split_large_functions(self, current, source):
