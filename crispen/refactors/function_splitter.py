...
def _update_best_split_candidate(best_idx, best_params, candidate_params, split_idx):
    if best_idx is None or len(candidate_params) < len(best_params):
        return split_idx, candidate_params
    ...
    for split_idx in valid_splits:
        ...
        if is_method and "self" in free:
            ...
            best_instance_idx, best_instance_params = _update_best_split_candidate(
                best_instance_idx, best_instance_params, params_no_self, split_idx
            )
        else:
            best_static_idx, best_static_params = _update_best_split_candidate(
                best_static_idx, best_static_params, free, split_idx
            )
...
class FunctionSplitter(Refactor):
    ...
    def _analyze(self, source: str) -> None:
        ...
        return self._plan_function_splits(source)

    def _plan_function_splits(self, source):
        current = source
