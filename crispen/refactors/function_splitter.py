...
def _update_best_candidate(current_params, current_idx, best_idx, best_params):
    if best_idx is None or len(current_params) < len(best_params):
        return current_idx, current_params
    ...
    * ``is_instance_method=True`` — the tail references ``self``, so the
    ...
    for split_idx in valid_splits:
        ...
        if is_method and "self" in free:
            ...
            best_instance_idx, best_instance_params = _update_best_candidate(
                params_no_self, split_idx, best_instance_idx, best_instance_params
            )
        else:
            best_static_idx, best_static_params = _update_best_candidate(
                free, split_idx, best_static_idx, best_static_params
            )
...
class FunctionSplitter(Refactor):
    ...
    def _analyze_and_split_functions(self, source):
        current = source
        return self._split_functions_iteratively(current, source)

    def _split_functions_iteratively(self, current, source):
