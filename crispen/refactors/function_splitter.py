...
def _update_best_split_choice(best_idx, best_params, candidate_idx, candidate_params):
    if best_idx is None or len(candidate_params) < len(best_params):
        return candidate_idx, list(candidate_params)
    ...
    * ``is_instance_method=True`` — the tail references ``self``, so the
    ...
    for split_idx in valid_splits:
        ...
        if is_method and "self" in free:
            ...
            best_instance_idx, best_instance_params = _update_best_split_choice(
                best_instance_idx,
            ...
            best_static_idx, best_static_params = _update_best_split_choice(
                best_static_idx,
                best_static_params,
                split_idx,
                free,
...
class FunctionSplitter(Refactor):
    ...
    def _split_oversized_functions(self, source):
        current = source
        return self._iteratively_split_oversized_functions(current, source)

    def _iteratively_split_oversized_functions(self, current, source):
