...
def _update_best_split(best_idx, best_params, cand_idx, cand_params):
    if best_idx is None or len(cand_params) < len(best_params):
        return cand_idx, cand_params
...
class FunctionSplitter(Refactor):
    ...
    def _split_large_functions(self, source):
        current = source
        return self._split_large_functions_iter(current, source)

    def _split_large_functions_iter(self, current, source):
