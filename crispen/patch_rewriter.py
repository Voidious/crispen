...
_PATCH_RULES = (
    ...
    "   - If F was **not migrated**: go to step 2b.\n"
    "2b. F stayed in the original module but may **call** migrated helpers that "
    "use the patched name in their new module. Find the patched name N "
    "(last component, e.g. `call_with_tool` in `pkg.module.call_with_tool`).\n"
    "   - Check the **Name references** section of each new file. "
    "If N is listed there, it means that file imports N and certain migrated "
    "entities use it.\n"
    "   - Look at the modified original file's source: does F call any of those "
    "listed entities (directly or via intermediate calls)? "
    "If yes, set M = that new file's module and go to step 3.\n"
    "   - If N does not appear in any new file's name references, or F does not "
    "call any entity that uses N: leave the patch unchanged.\n"
    ...
    "```\n\n"
    "**Example B — F not migrated but calls a migrated helper:**\n"
    "```python\n"
    "# Before split: run_engine() in crispen.engine calls _apply_foo() which uses "
    "call_with_tool; both in same module\n"
    "@patch('crispen.engine.call_with_tool')  # correct before split\n\n"
    "# After split: run_engine() STAYS in crispen.engine.__init__\n"
    "#              _apply_foo() MOVES to crispen.engine.helpers\n"
    "# crispen/engine/helpers.py: from ...llm_client import call_with_tool\n"
    "# run_engine() calls _apply_foo() which now resolves call_with_tool in helpers\n"
    "# Name references for helpers.py: call_with_tool → _apply_foo\n"
    "@patch('crispen.engine.helpers.call_with_tool')  # correct after split\n"
    "# WRONG: @patch('crispen.engine.call_with_tool')  "
    "-- run_engine didn't migrate, but its call path now resolves "
    "call_with_tool in helpers, not in __init__\n"
    "```\n"
)
...
def _build_classify_prompt(
    ...
    parts.append(
        ...
        "   - If F was **not migrated**: go to step 2b.\n"
        "2b. F stayed in the original module but may call migrated helpers that "
        "now own the patched name. Find the patched name N (last component). "
        "Check the **Name references** in the context above for each new file. "
        "If N is listed in a new file's Name references, look at the modified "
        "original file source: does F call any of those listed entities? "
        "If yes, set M = that new file's module and go to step 3. "
        "If no, include the path unchanged in `patch_renames`.\n"
        "3. Find the patched name N (last component, e.g. `MetadataWrapper` in "
