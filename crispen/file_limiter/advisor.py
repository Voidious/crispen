...
def _init_placements_and_ids(result):
    if result is None:
        return None
...
def _assign_placements_chunk(
    ...
    init_vals = _init_placements_and_ids(result)
    if init_vals is None:
        return None
    placements, placed_ids = init_vals
...
def _rename_conflicting_chunk(
    ...
    return _rename_conflicting_placements(chunk, forbidden_files, n_groups, result)


def _rename_conflicting_placements(chunk, forbidden_files, n_groups, result):
    init_vals = _init_placements_and_ids(result)
    if init_vals is None:
        return None
    placements, placed_ids = init_vals
...
def _assign_placements(
    ...
    for chunk_start in range(0, len(groups_to_place), _PLACEMENT_CHUNK_SIZE):
        ...
        # Scale min_files to this chunk's share of total lines.  When
        # total_lines is zero (all entities have zero-length ranges) fall
        ...
        if total_lines > 0:
            ...
            chunk_min_files = max(2, math.ceil(min_files * chunk_lines / total_lines))
        else:
