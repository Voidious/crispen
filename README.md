...
- `file_limiter_patch_update` — controls how `@patch` string literals are updated after FileLimiter moves entities: `"ignore"` (leave all patch strings as-is) or `"direct"` (scan every `*.py` file in the repo and update string literals whose value is `"old.module.EntityName"` or starts with `"old.module.EntityName."`, replacing the old module prefix with the new one). Only strings where the segment immediately after the old module path is a moved entity name are touched; imported dependencies that ended up in multiple new files are left unchanged. Default: `"ignore"`.
