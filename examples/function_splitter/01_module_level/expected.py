# After splitting: the persistence phase moves to a private helper.
# (Exact split point and parameter list depend on free-variable analysis.)
def _persist_results(transformed, db, config):
    db.begin()
    inserted = 0
...
def process_batch_records(records, db, config):
    ...
    return _persist_results(transformed, db, config)
